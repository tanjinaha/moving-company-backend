...
public class CustomerService {
    ...
    // 5. UPDATE an existing customer (like changing their email or phone)
    public Customer updateCustomer(Customer customer) {
        // This will update the customer if the ID exists, or create a new one if it doesn’t
        return customerRepository.save(customer);
    }

    // 6. SEARCH customers by name (like searching for "mo" or "Ali")
    public List<Customer> searchCustomersByName(String name) {
        // This returns all customers whose name contains the given text (case-insensitive)
